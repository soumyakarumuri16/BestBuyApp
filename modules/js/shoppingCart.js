...
{
	var lPrice = "";
	if(frmProductDetail.lblPrice.skin == sknLblRedFont)
	{
		lPrice = frmProductDetail.lblPrice.text.substring(10);
	}
	else if(frmProductDetail.lblPrice.skin == sknLblBlackFont)
	{
		lPrice = frmProductDetail.lblPrice.text.substring(1);
	}
...
function populateShoppingCart()
{
	...
	if(lData.length > 0)
	{
		...
		for(var i=0;i < lData.length;i++)
		{
			...
			"lblPrice":{text:"$"+lData[i]["price"],skin:lData[i]["priceSkin"]},
			"btnDelete":".",
		...
		if(lFlag)
		{
			frmShoppingCart.lblNewProdMsg.text = "You have items that are new. Shipping may be delayed";
		}
		else
		{
			frmShoppingCart.lblNewProdMsg.text = "Normal shipping schedule";
		}
